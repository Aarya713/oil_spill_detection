    ...
    if not os.path.exists('classifier_model.h5'):
        try:
            ...
            if os.path.exists('classifier_model.h5'):
                ...
                st.sidebar.error("❌ Classifier download failed")
        except Exception as e:
            st.sidebar.error(f"❌ Classifier download failed: {e}")
    ...
    if not os.path.exists('segmentation_model.h5'):
        try:
            ...
            if os.path.exists('segmentation_model.h5'):
                ...
                st.sidebar.error("❌ Segmentation download failed")
        except Exception as e:
            st.sidebar.error(f"❌ Segmentation download failed: {e}")
...
class OilSpillDetector:
    def __init__(self):
        ...
        if TENSORFLOW_AVAILABLE:
            ...
            st.error("❌ TensorFlow not available - running in demo mode")
    ...
    def segment_image(self, image):
        ...
        try:
            ...
            if OPENCV_AVAILABLE:
                ...
                mask_pil = mask_pil.resize((original_size[1], original_size[0]))
...
def show_home():
    ...
    st.sidebar.write(f"TensorFlow: {'✅ Available' if TENSORFLOW_AVAILABLE else '❌ Not Available'}")
    st.sidebar.write(f"OpenCV: {'✅ Available' if OPENCV_AVAILABLE else '❌ Not Available'}")
